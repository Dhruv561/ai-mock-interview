...
- **Reconnects still likely lose transcription for the rest of the interview — half-fixed by Feature 20, not fully.** Feature 20's disconnect-cleanup fix made `session.resume` recreate the STT session (`backend/app/websocket/interview.py`), which is necessary but not sufficient: the extension's `MediaRecorder` is never restarted on reconnect, so the WebM container header (only present in the very first chunk) was already sent to the *old*, now-closed STT session — a fresh Deepgram connection fed the recorder's ongoing headerless mid-stream chunks will most likely still fail to decode and close again. The interview itself survives either way (the STT guard keeps it alive), but nothing transcribes after a drop. Remaining fix: restart `MediaRecorder` on reconnect (extension-side) so each new STT session gets a self-contained stream. Not attempted yet — deliberately not half-built further without a real Deepgram key to verify against.
...
**Gemini Live API (alternative realtime pipeline) — IMPLEMENTED, untested against live API.**

Mirrors the ElevenLabs Conversational AI pattern from an unmerged branch: a complete alternative interview pipeline using Google Gemini's Live WebSocket API (bidirectional, native audio I/O, no client-side transcoding needed) instead of the real pipeline's Deepgram STT → Claude interviewer → ElevenLabs TTS. A candidate codebases this against the same backend; final reviews are generated from the thin transcript-only evidence using the real evaluator (not a separate mock path).

- **Backend**: `app/api/gemini_live.py` (two endpoints: `GET /token` mints ephemeral tokens via REST, `POST /review` generates final reviews from transcripts); `app/providers/realtime/gemini.py` (ephemeral-token fetch and WebSocket URL builder using stdlib `urllib.request` + `asyncio.to_thread` per the no-new-dependencies principle); `backend/tests/test_gemini_live.py` (7 tests, all passing, covering auth gating, token minting, transcript validation, review generation).
- **Extension**: `content/geminiLiveSession.ts` (session lifecycle, audio-to-Gemini pipeline, transcript accumulation, POST to review endpoint at end); `media/geminiLiveMicrophone.ts` (16kHz mono PCM16 capture via `ScriptProcessorNode` at fixed sample rate, mirroring privacy invariants from real `microphone.ts`); `networking/geminiLiveSocket.ts` (JSON wire-protocol layer, emits typed events for audio/transcript deltas); `components/GeminiLivePanel.tsx` (simplified UI, no interview-stage machine or rubric); wired into `App.tsx` behind `VITE_USE_GEMINI_LIVE=true` flag.
- **Verified** (environment-only): backend 216/216 tests pass (7 new for Gemini Live endpoints), extension TypeScript builds, no secrets leaked. **Unverified**: live Gemini connection (requires `GEMINI_API_KEY`); WebSocket protocol event names/field casing transcribed from 2026 docs (may need small fixes if the live API differs).
- **Key decisions**: uses existing `portSocketFactory` relay unchanged (Gemini Live WebSocket connects server-side from service worker, same as real backend); REST calls (token mint) relay via new `chrome.runtime.onMessage` handler in background worker; `generativelanguage.googleapis.com` added to manifest host permissions; interview state is client-only (no server persistence), only transcript is logged for review.
...
| 2026-09-13 | ~~The floating layout (1b) stays inside the existing reserved 420px column rather than the design's full-viewport-width overlay~~ — **reversed same day** after user feedback that floating still looked like the sidebar, not a bar above the code | `content/App.tsx` gained a `PanelShell` that picks the outer container per layout: docked/split keep the fixed full-height right column; floating-while-active gets a fixed bottom-anchored bar and releases the reserved page margin instead. Still one page-space model active at a time, just chosen dynamically rather than fixed at mount — see `architecture.md`'s same-day addendum |
