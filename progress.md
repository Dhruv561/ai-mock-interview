...
**Live rehearsal session (2026-09-13) — all three real provider keys exercised together for the first time, two real defects found and fixed:**

- **Service-worker eviction was silently reconnecting the WS every 5-10s.** First live run with real Deepgram/Anthropic/ElevenLabs keys produced garbled transcripts ("How do I purchase po?" instead of "How do I approach this problem?") and interviewer audio that never played. Diagnosed via Chrome's own devtools protocol — the extension's service-worker devtools target id changed mid-session, and the backend log showed 8 separate `/ws/interview` connections accepted in a 45s session. Root cause: an open WebSocket inside an MV3 service worker doesn't count as "activity" against Chrome's ~30s idle-eviction timer, so the worker (and the WS it owns) was being killed and respawned repeatedly — each reconnect forced a brand-new headerless Deepgram session mid-utterance and could orphan an in-flight TTS audio bracket. Not the same bug as the "reconnects lose transcription" item below, but the same failure mode, hit far more often than expected. Fixed with a `chrome.alarms` keepalive scoped to the interview port's lifetime (`extension/src/background/index.ts`); verified stable for a full 60s idle window afterward with zero restarts. The real ElevenLabs provider itself was verified working correctly in isolation before this diagnosis (3 audio chunks, ~2.8s of audio, no errors) — ruling out provider latency as the cause.
- **Interviewer pacing was a flat 30s cooldown, then a flat 2s cooldown, now dynamic.** The 30s floor (original Feature 08 design) produced 35-50s silences before a reply — read live as "not conversational, unlike Claude/Gemini live chat." Lowering it to a flat 2s then made the interviewer just as quick to interrupt a candidate mid-monologue. Replaced with two named cooldowns in `interview/controller.py`: `REACTIVE_COOLDOWN_SECONDS` (2s) when the candidate is answering a pending question or directly addressing the interviewer (`InterviewController.awaiting_response` / a cheap question-mark-or-address-word heuristic), `OBSERVATION_COOLDOWN_SECONDS` (15s) otherwise (narrating/coding). `code_update` never counts as conversational. 6 new tests; 212/212 backend + ruff clean.
...
- **Reconnects lose transcription for the rest of the interview.** The backend only creates an STT session on `session.start`, not on `session.resume`, and by then the recorder is mid-stream so a fresh Deepgram connection would get headerless audio and close. The interview itself survives (the STT guard keeps it alive), but nothing transcribes after a drop. Fix: restart `MediaRecorder` on reconnect so each STT session receives a self-contained WebM stream, and recreate the STT session on resume. Not attempted yet — deliberately not half-built. **Much less likely to bite now** that the 2026-09-13 service-worker keepalive fix (below) has stopped reconnects from happening every 5-10s on their own, but a genuine network drop would still hit this.
- **FIXED 2026-09-13 — the extension's MV3 service worker was being evicted mid-interview, forcing far more frequent reconnects than the item above assumed.** See the "Live rehearsal session" entry under "In progress" for the full diagnosis and fix (`chrome.alarms` keepalive in `extension/src/background/index.ts`).
