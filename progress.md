...
## Status: default STT provider swapped to ElevenLabs — Feature 05 reopened to IMPLEMENTED pending a live key check

Last updated: 2026-09-13 (later the same day)

> **STT provider swap, at the user's request: Deepgram → ElevenLabs realtime.** Checked ElevenLabs' actual API before writing code and found it wasn't a same-shape swap — their realtime STT websocket has no container-autodetection mode (unlike Deepgram), only raw PCM/µ-law. Asked the user how to handle that; they chose the full swap over a batch-mode workaround or a backend-only scaffold, so the extension's mic capture pipeline (`extension/src/media/microphone.ts`) was rebuilt from `MediaRecorder`/WebM to a Web Audio PCM16/16kHz graph (`ScriptProcessorNode`, deliberately not `AudioWorklet` — its `addModule()` is a fetch-like content-script call, and §B.1 already found leetcode.com's CSP kills that class of activity). Deepgram's own connection string was updated to match the new raw-PCM reality and stays selectable via `STT_PROVIDER=deepgram`. New `backend/app/providers/stt/elevenlabs.py`, `Settings.stt_provider` selector, and simplified (no-longer-WebM-header-dependent) audio-buffer trimming in `networking/websocket.ts`. Full backend suite (209/209) and extension suite (140/140) plus typecheck/lint/build/`scripts/check.sh` all pass; not yet verified against a real ElevenLabs connection (no key in this environment) — same live-verification gap Deepgram itself had before a key was supplied. **Feature 05 reopened from `DONE` to `IMPLEMENTED`** — see its dated update in `FEATURE_PROGRESS.md` and `architecture.md` §H.1 for the full record.
>
