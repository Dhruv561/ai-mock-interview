...
## Status: default STT provider swapped to ElevenLabs, live-verified end-to-end — Feature 05 back to DONE

Last updated: 2026-09-13 (evening)

> **STT provider swap, at the user's request: Deepgram → ElevenLabs realtime — now live-verified, not just implemented.** Checked ElevenLabs' actual API before writing code and found it wasn't a same-shape swap — their realtime STT websocket has no container-autodetection mode (unlike Deepgram), only raw PCM/µ-law. Asked the user how to handle that; they chose the full swap over a batch-mode workaround or a backend-only scaffold, so the extension's mic capture pipeline (`extension/src/media/microphone.ts`) was rebuilt from `MediaRecorder`/WebM to a Web Audio PCM16/16kHz graph (`ScriptProcessorNode`, deliberately not `AudioWorklet` — its `addModule()` is a fetch-like content-script call, and §B.1 already found leetcode.com's CSP kills that class of activity). Deepgram's own connection string was updated to match the new raw-PCM reality and stays selectable via `STT_PROVIDER=deepgram`. New `backend/app/providers/stt/elevenlabs.py`, `Settings.stt_provider` selector, and simplified (no-longer-WebM-header-dependent) audio-buffer trimming in `networking/websocket.ts`.
>
> **Live demo run the same day, at the user's request**: backend started from this worktree with the user's own real `ELEVENLABS_API_KEY`/`ANTHROPIC_API_KEY`, Chrome launched with the built extension on a real `leetcode.com/problems/two-sum` page. Inspected via Chrome DevTools Protocol against the backend's own debug log (not just eyeballing the UI): confirmed properly-shaped 8000-byte PCM16 audio frames arriving, real ElevenLabs STT correctly transcribing actual speech ("Hello, can you hear me?") through `transcript.partial`→`transcript.final`, a real (non-canned) Anthropic interviewer question about Two Sum specifically, and real ElevenLabs TTS framing. **Found and fixed one operational hazard along the way** (not a code defect): a stale `uvicorn` process from an earlier session was already squatting on port 8000, serving old pre-swap code from the main checkout — a plain `curl /health` didn't reveal this, only checking which binary actually owned the port did. Also tried `MIN_COOLDOWN_SECONDS` at 2.0 (from 30.0) for faster live iteration, at the user's request, then reverted to 30.0 once confirmed working — 2s made the interviewer interject far more than a real interview would.
>
> **Full backend suite (209/209) and extension suite (140/140) plus typecheck/lint/build/`scripts/check.sh` all pass. Feature 05 flipped back to `DONE`** — see its dated updates in `FEATURE_PROGRESS.md` and `architecture.md` §H.1 for the full record.
>
> **Previously (still true of everything not superseded above):**
...
Worth doing whenever keys become available: exercise the real ElevenLabs STT (or Deepgram, via `STT_PROVIDER=deepgram`) and Anthropic providers, which would let Features 05 and 08 flip to `DONE`.
...
- **Reconnects lose transcription for the rest of the interview.** The backend only creates an STT session on `session.start`, not on `session.resume`. The interview itself survives (the STT guard keeps it alive), but nothing transcribes after a drop. (Updated 2026-09-13: since the mic capture pipeline moved to raw PCM16 — see the STT provider swap below — there's no longer a WebM-header reason this specifically breaks reconnects; it's just that no code recreates the STT session on `session.resume` at all. Fix: recreate the STT session on resume.) Not attempted yet — deliberately not half-built.
...
- No external API keys configured. STT (ElevenLabs by default, Deepgram as a fallback), interviewer LLM (Anthropic) and — from Phase 7 — TTS (ElevenLabs) are all implemented behind provider interfaces but unverified without real keys; mock providers cover the happy path (`architecture.md` §S).
...
| 2026-09-13 | Default STT provider swapped Deepgram → ElevenLabs realtime (Deepgram kept as `STT_PROVIDER=deepgram` fallback) | User request. Forced mic capture to switch from `MediaRecorder`/WebM to raw PCM16/16kHz (`ScriptProcessorNode`, not `AudioWorklet` — its `addModule()` fetch would hit the same page-CSP wall §B.1 already found for the WebSocket), since ElevenLabs' realtime endpoint has no container-autodetection mode. Full record: `architecture.md` §H.1, `FEATURE_PROGRESS.md` Feature 05's dated update |
