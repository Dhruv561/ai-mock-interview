...
## Spike: OpenAI Realtime API (alternative interviewer pipeline)

**IMPLEMENTED but not integrated.** Experimental/spike-grade work — a complete alternative to the real Deepgram+Claude+ElevenLabs pipeline: direct-to-OpenAI Realtime API connection, client-side, ephemeral auth, PCM16 mic capture at 24kHz, live WebSocket event handling (input audio send, output audio/transcript recv), and evidence-grounded final review via the real evaluator.

**What was built:**
- Backend: `/api/openai-realtime/session` (GET, mints ephemeral tokens), `/api/openai-realtime/review` (POST, generates review). Gated with shared-secret-token auth like the rest of the backend.
- Extension: `openaiRealtimeSession.ts` (lifecycle), `openaiRealtimeSocket.ts` (OpenAI wire protocol), `pcm16Microphone.ts` (PCM16 capture at 24kHz per OpenAI's format), `OpenAIRealtimePanel.tsx` (minimal UI — no state machine/rubric/hints, just Start/End and live transcript).
- Networking: Extended `PortSocket` + background relay to support WebSocket subprotocols (ephemeral key auth), added background HTTP relay for REST calls (CSP workaround).
- Config: `OPENAI_API_KEY`, `OPENAI_REALTIME_MODEL` (default `gpt-realtime-2.1`) in backend; `VITE_USE_OPENAI_REALTIME` flag (off by default) in extension to mount the alternative panel instead of the real one.
- Tests: 6 backend endpoint tests (auth, minting, review generation); typecheck, lint, and build all pass.

**Why a spike:** This wasn't a feature request — it's exploring whether OpenAI's Realtime API (which handles STT/TTS/LLM as one hosted service) can replace the three-provider real pipeline. Useful for performance testing and as a contingency if Deepgram/Anthropic/ElevenLabs keys aren't available. Same evidence-grounded review engine (reuses the real evaluator) means the final feedback is still grounded in transcript, not generic.

**Next step if pursuing:** Add `OpenAI_API_KEY` to the deployment, build with `VITE_USE_OPENAI_REALTIME=true`, and manually test against a real OpenAI account — transcript rendering, turn detection, review generation. Current implementation has no known blockers but is untested against the live API.

**Verified:** Backend 215/215 tests pass (6 new); extension typecheck, lint (`1 pre-existing warning`), build all pass.
