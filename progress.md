...
**Feature 17 (Backend containerization and CI/CD) — VERIFIED, backend is now live on the user's own VPS.** `backend/Dockerfile` + `docker-compose.yml` built and verified locally first; the backend now also has a judges-only auth gate (`SESSION_SHARED_SECRETS`, checked at the WS handshake before `accept()`) and two cost-control limits (`MAX_CONCURRENT_SESSIONS`, `SESSION_MAX_DURATION_SECONDS`) — all default to off, so nothing changed for local dev; covered by `backend/tests/test_session_auth_and_limits.py` (7 tests, full suite 126/126 pass).
