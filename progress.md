...
## Decisions log (Feature 18 — panel layout presets)
...
| 2026-09-13 | Merged into `main` well after Features 11–17 had landed on it (this branch diverged right after Feature 10) — renumbered from its own "Feature 17" to 18 to avoid colliding with the also-outstanding resizable-panel worktree, which used that number too | `InterviewPanel.tsx`'s mic-permission-blocking gate (Feature 16) and screen-capture wiring (Feature 12) were manually re-applied on top of the new layout-switcher structure rather than dropped; `PanelBodyProps` gained a real `screenStatus` and `DockedPanel`/`SplitPanel` now surface it — Feature 12 didn't exist when this branch was built, so its own "no screen signal yet" reasoning for omitting it was stale by merge time |

## Spike (not a feature — throwaway, branch `worktree-spike-elevenlabs-convai`)

| Date | Decision | Why |
|---|---|---|
| 2026-09-13 | Built a standalone, disconnected spike (`spikes/elevenlabs-convai/`) comparing ElevenLabs Conversational AI (one hosted agent doing STT+LLM+TTS+turn-taking itself) against the real Deepgram→Claude-controller→ElevenLabs-TTS pipeline in `architecture.md` §J/§L | Requested to check whether a fully-managed conversational API "knows when to be quiet" well enough to be worth losing the product's own deterministic controller (CLAUDE.md #4). Kept fully outside `backend/`/`extension/` so it can be deleted with zero risk to the real app; not entered in `FEATURE_PROGRESS.md` since it isn't on the roadmap |
| 2026-09-13 | Not wired into the extension/LeetCode page at all — a plain static page + a ~40-line FastAPI relay that only proxies ElevenLabs' `get-signed-url` call (keeps `ELEVENLABS_API_KEY` server-side, same rule as CLAUDE.md #7) | Fastest way to answer the conversational-feel question; the SDK (`@elevenlabs/client`, loaded via jsDelivr) already handles mic capture/playback/turn-taking, so no code extraction, hints, or rubric plumbing was needed to get a real answer |
| 2026-09-13 | Verdict pending manual test — agent created and signed-url relay verified working end-to-end (see `spikes/elevenlabs-convai/README.md`), but judging turn-taking quality needs an actual person talking to it | Not something that can be verified by an automated check; next step is opening `spikes/elevenlabs-convai/web/` in a browser and talking to it |
| 2026-09-13 | Wired the same pipeline into the real LeetCode extension as a second, opt-in panel (`components/ConvaiInterviewPanel.tsx`, `VITE_USE_ELEVENLABS_CONVAI=true` in `content/App.tsx`) instead of leaving it standalone | Requested — the standalone page above answers "does the raw conversational feel work", but judging it as a replacement needs the real Start/Hint/End buttons, real code-editor context, and real panel UI, not a bare test page |
| 2026-09-13 | Hand-rolled the ElevenLabs Conversational AI WebSocket protocol (`networking/convaiSocket.ts`) rather than using the official `@elevenlabs/client` SDK inside the extension | That SDK insists on owning mic capture and the socket together in one JS context; `architecture.md` §B.1 already forced this app's real WebSocket into the background service worker because leetcode.com's CSP kills a page-context connection to an external host — the same constraint applies to a Convai connection, so the SDK's own high-level class can't be dropped in as-is. The wire protocol was read out of the actual shipped SDK source (`@elevenlabs/client@1.25.0`'s `dist/BaseConversation.js`/`WebSocketConnection.js`/`attachInputToConnection.js`, pulled via `npm pack`), not guessed from ElevenLabs' docs, which didn't give exact field-level shapes at the time |
| 2026-09-13 | Mic capture for this pipeline is a new raw 16kHz PCM16 module (`media/convaiMicrophone.ts`) instead of reusing `media/microphone.ts`'s WebM/Opus `MediaRecorder` capture | ElevenLabs Conversational AI's `user_audio_chunk` wire format is raw PCM, not a WebM container — a genuinely different format, not just a refactor target. The new module still mirrors `microphone.ts`'s privacy-critical invariants line for line (architecture.md §B.2: at most one capture, an unconditional kill switch, idempotent stop) since that rule isn't specific to one pipeline |
| 2026-09-13 | Reused `media/interviewerAudioPlayer.ts` unchanged for playback rather than forking it, translating Convai's `"pcm_16000"` format string to its expected `"pcm_s16le_16000"` in `media/useConvaiAudioPlayback.ts` instead | Same underlying bytes (16-bit signed PCM, mono, platform/little-endian — confirmed against the real SDK source), different naming convention between ElevenLabs' TTS product and its Conversational AI product. A one-line string translation was simpler and lower-risk than maintaining a second Web-Audio-scheduling implementation |
| 2026-09-13 | This pipeline has no per-utterance "audio done" event (unlike the real pipeline's `interviewer.audio.end`) — the agent just stops sending `audio` events — so `useConvaiAudioPlayback.ts` treats a 600ms gap between chunks as the end of a turn | Documented as a heuristic, not a protocol guarantee; acceptable for a throwaway comparison, called out explicitly in code so it isn't mistaken for real protocol behaviour later |
| 2026-09-13 | Added `backend/app/api/convai.py` (two endpoints: `GET /signed-url`, `POST /review`) to the **real** backend rather than keeping everything in `spikes/elevenlabs-convai/`'s standalone server | The extension needs the real backend running anyway (LeetCode page, real dev workflow); `/review` also deliberately reuses the real `agents/evaluator.py` unchanged, feeding it a minimal `InterviewState` built from a client-captured transcript, so this pipeline still produces a genuine evidence-based review (CLAUDE.md §10) instead of skipping review entirely. Both endpoints verified live against the real ElevenLabs API and the real evaluator (mock LLM provider) before this commit |
| 2026-09-13 | No screen capture, no live interview stage, and no rubric in `ConvaiInterviewPanel.tsx` | Out of scope for what this spike is measuring (turn-taking feel) — `PanelBodyProps.stage` is passed `null` and `screenStatus` a static `"idle"`, both already-optional per that interface, rather than building a second stage machine or rubric source for a pipeline being evaluated on conversational feel alone |
| 2026-09-13 | Full verification for this slice: backend `pytest` (204 passed, no change), `ruff check` (clean), extension `tsc --noEmit` + `eslint` (clean, one pre-existing unrelated warning) + `vitest` (138 passed, no change), plus a live curl of both new endpoints against the real ElevenLabs API. No new automated tests were written for the new spike-only files themselves | Consistent with this being an explicitly throwaway comparison branch (see the brainstorming/spike framing above) rather than a roadmap feature — the verification bar was "does the real app still work, and do the new endpoints actually work," not full coverage of code that may be deleted once the comparison is judged |
