...
- **Done when:** every row above has a corresponding manual test performed at least once before Feature 16 is marked done. Automated coverage that already exists, so the manual pass is confirmation rather than first exercise: screen-capture denial (`extension/src/media/screen.test.ts`, `useScreenCapture.test.ts`), the STT provider-*start*-failure path (`backend/tests/test_websocket_interview.py::test_stt_provider_start_failure_degrades_gracefully`), TTS failure (`backend/tests/test_interviewer_audio.py`), and WebSocket disconnect/reconnect/resume (`extension/src/networking/websocket.test.ts`, `backend/tests/test_websocket_interview.py`'s resume tests). Real gaps with no automated coverage anywhere: the STT *send*-failure path that actually emits `stt_unavailable` (only the start-failure path above is tested; `backend/tests/test_stt_providers.py` covers provider selection only, not this WS-level degrade path), and the LeetCode bridge-vs-DOM-fallback extraction logic itself (`content/editor.ts`) — only the call-site graceful-`null` behavior is unit-tested (`content/interviewSession.test.ts`), per §D's own note that live DOM shape isn't meaningfully unit-testable. Microphone-denied coverage is being added concurrently as of this writing.
